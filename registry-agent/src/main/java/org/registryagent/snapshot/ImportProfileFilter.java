...
public class ImportProfileFilter {
    ...
    public static CharacterPayload apply(CharacterPayload source, ImportProfile profile) {
        ...
        out.setInventory(profile.isImportInventory() ? source.getInventory() : null);

        out.setBank(profile.isImportBank() ? source.getBank() : null);
    ...
    private static CharacterIdentity applyIdentityCap(CharacterIdentity src, ImportProfile profile) {
        ...
        log.info("ImportProfileFilter: capping level " + src.getLevel() + " → " + cap
    ...
    private static Currency applyCurrencyCap(Currency src, ImportProfile profile) {
        ...
        if (values == null || !values.containsKey("copper")) return src;
        ...
        log.info("ImportProfileFilter: capping gold " + (copper / 10_000) + "g → "
                 + (cap / 10_000) + "g (profile=" + profile.getName() + ")");
    ...
    private static Map<String, Map<String, Integer>> filterSkills(
            Map<String, Map<String, Integer>> src, ImportProfile profile) {

        if (src == null) return null;

        Set<Integer> blocked = profile.getBlockedSkillIds();
        if (blocked == null || blocked.isEmpty()) return src;
    ...
    private static Map<String, Map<String, Integer>> filterReputation(
            Map<String, Map<String, Integer>> src, ImportProfile profile) {
