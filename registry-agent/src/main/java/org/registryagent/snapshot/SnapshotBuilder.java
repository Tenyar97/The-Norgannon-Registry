...
public class SnapshotBuilder {
    ...
    public SnapshotRecord build(LoginSession session,
        ...
        if (canonicalJson == null) {
            throw new IllegalStateException(
                    "Failed to serialize payload for signing — characterId=" +
                    session.getCharacterId());
        }

        record.setServerSignature(signer.sign(canonicalJson));

        log.fine("Built snapshot — characterId=" + session.getCharacterId() +
    ...
    public void seedSequence(String characterId, long lastKnownSequence) {
        ...
        log.fine("Seeded sequence for characterId=" + characterId +
                 " at " + lastKnownSequence);
    }
