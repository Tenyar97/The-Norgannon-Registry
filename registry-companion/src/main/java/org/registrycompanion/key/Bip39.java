...
public class Bip39 {
    ...
    public static String encode(byte[] entropy) {
        if (entropy.length != 32) {
            throw new IllegalArgumentException("Entropy must be exactly 32 bytes");
        }

        byte[] hash     = sha256(entropy);
        int    checksum = (hash[0] & 0xFF) >> 0;  

        byte[] combined = Arrays.copyOf(entropy, 33);
        combined[32]    = hash[0]; 

        int[]  indices = extractIndices(combined, 24);
        String[] words = new String[24];

        for (int i = 0; i < 24; i++) {
            words[i] = WORDLIST[indices[i]];
        }
    ...
    public static byte[] decode(String phrase) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (idx < 0) {
                throw new IllegalArgumentException(
                    "Word not in BIP-39 list: '" + words[i] + "' (word " + (i + 1) + ")");
            }
            indices[i] = idx;
        }

        byte[] combined = packIndices(indices, 33);

        byte[] entropy = Arrays.copyOf(combined, 32);
        byte[] hash    = sha256(entropy);

        if ((combined[32] & 0xFF) != (hash[0] & 0xFF)) {
            throw new IllegalArgumentException(
                "Recovery phrase checksum invalid - check for typos");
        }

        return entropy;
    }

    private static int[] extractIndices(byte[] data, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            int b0 = bytePos     < data.length ? (data[bytePos]     & 0xFF) : 0;
            int b1 = bytePos + 1 < data.length ? (data[bytePos + 1] & 0xFF) : 0;
            int b2 = bytePos + 2 < data.length ? (data[bytePos + 2] & 0xFF) : 0;

            int window = (b0 << 16) | (b1 << 8) | b2;
            indices[i] = (window >> (13 - bitOff)) & 0x7FF;
        }
        return indices;
    }

    private static byte[] packIndices(int[] indices, int byteCount) {
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            int val    = indices[i] & 0x7FF;
            int window = val << (13 - bitOff);

            if (bytePos < result.length)     result[bytePos]     |= (byte)((window >> 16) & 0xFF);
            if (bytePos + 1 < result.length) result[bytePos + 1] |= (byte)((window >> 8)  & 0xFF);
            if (bytePos + 2 < result.length) result[bytePos + 2] |= (byte)(window         & 0xFF);
        }
        return result;
    }
    ...
    public static final String[] WORDLIST = {
    		...
    		"turkey","turn","turtle","twelve","twenty","twice","twin","twist","two","type","typical",
